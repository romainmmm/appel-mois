...
def build_timesheet_workbook(dates, employees, timesheet_data, output_path):
    ...
    """
    from timesheet import worked_hours, period_total, period_tips, get_entry
    ...
    headers = ["Employé", "Rôle", "Total heures", "Total pourboires ($)"]
    for col, label in enumerate(headers, start=1):
    ...
    for r, (name, role) in enumerate(employees, start=3):
        ...
        tips = period_tips(timesheet_data, name, dates)
        grand_total += total
        grand_tips += tips
        for col, val in enumerate([name, role or "", total, tips], start=1):
        ...
        summary.cell(row=r, column=4).number_format = "0.00"

    tr = len(employees) + 3
    summary.cell(row=tr, column=2, value="TOTAL").font = Font(bold=True)
    for col, val in ((3, grand_total), (4, grand_tips)):
        tc = summary.cell(row=tr, column=col, value=round(val, 2))
    ...
    dheaders = ["Employé", "Date", "Jour", "Arrivée", "Départ",
                "Pause (min)", "Heures", "Pourboires ($)"]
    ...
    for name, _role in employees:
        for d in dates:
            ...
            tips = e.get("tips", 0)
            if not e["arrivee"] and not e["depart"] and not h and not tips:
                continue  # skip fully-empty days in the detail
            vals = [name, d.strftime("%d/%m/%Y"), WEEKDAYS_FR[d.weekday()],
                    e["arrivee"], e["depart"], e.get("pause", 0), h, round(tips, 2)]
            for col, val in enumerate(vals, start=1):
                c = detail.cell(row=row, column=col, value=val)
                c.border = _BORDER
                c.alignment = Alignment(horizontal="center", vertical="center")
            detail.cell(row=row, column=7).number_format = "0.00"
            detail.cell(row=row, column=8).number_format = "0.00"
            row += 1

    for col, w in zip("ABCDEFGH", [20, 12, 10, 9, 9, 12, 9, 14]):
